...
1. Loads **`testing`** (via `test-engineer` / `/test-ui`)
2. Plans tests: labels, validation error, pending submit, empty/error if async
3. Loads **`devtools`** when measuring — or marks Lighthouse/axe as `not evaluated` if tools did not run
4. Does **not** invent Lighthouse/axe numbers
5. Mentions reduced-motion only if animation is in scope (Info OK)

## Pass criteria

- [ ] Names `testing` (or test-engineer / `/test-ui` workflow)
- [ ] Concrete test cases for the form
- [ ] `devtools` used **or** honest `not evaluated` for measured scores
