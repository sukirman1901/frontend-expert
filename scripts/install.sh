...
Targets:
  ...
  cursor-user       Symlink pack skills + refs into ~/.cursor (no drift vs this repo)
...
sync_cursor() {
  ...
  echo "Synced skills + references + rule → $dest/.cursor/"
}
...
sync_cursor_user() {
  ...
  echo "Cursor user install OK → ~/.cursor (skills are symlinks into $ROOT)"
}
