...
if command -v jq >/dev/null 2>&1; then
  if jq -e '.version == "2.4.4" and (.skills | length == 33)' "$ROOT/plugin.json" >/dev/null; then
    ok "plugin v2.4.4 registers exactly 33 skills"
  else
    bad "plugin must be v2.4.4 with exactly 33 skills"
  fi
  for s in "${EXPECTED_SKILLS[@]}"; do
