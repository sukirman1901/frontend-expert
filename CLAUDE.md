...
3. Then → `design-tokens` (decision tree: custom → explicit → Plasma hard-gate → score; see `references/token-preset-scoring.md`) → `ui-components` → `anti-ai-slop` → `ui-feel` → `accessibility`
...
10. Before DONE on UI builds → **Conventions check** (`references/compliance-gates.md`)
