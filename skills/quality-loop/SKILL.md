...
\* Judgment only on blank-canvas / first pass (see `design-direction` skip rules).
...
| **Landing** | Marketing section stack via `marketing` (not hero-only; hand-roll) — or `n/a` for app-shell-only |
...
- [ ] Build chain includes **responsive** + **motion** (+ **marketing** if landing) or waivers
