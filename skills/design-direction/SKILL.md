...
- **Marketing / landing page** without a section recipe → load `marketing` after approaches (or with “langsung saja”)
- Multiple reasonable visual or interaction directions
- User ask is vague for a **new** screen (“bikin dashboard”, “UI modern”) — not mere “rapihin” on existing UI (that’s `polish` / `quality-loop`)
...
- User only wants micro polish on existing UI (“rapihin detail”, “feels off”) → use `polish`
- User wants iterate until audit-clean (“sampai bagus”) → use `quality-loop`, which may call judgment only on first blank-canvas pass
...
1. **Style inference** (for `tokens` / build) — if a project system exists:
...
Pair with `polish` / `anti-slop-design`. Full anti-patterns: `references/anti-patterns.md`.
