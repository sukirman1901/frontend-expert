...
Use browser/tooling evidence when available — don’t invent Lighthouse scores. Pair with `performance`, `accessibility`, and `testing`.
...
6. **Report** — tool · metric · value · source; hand remediations to `performance` / `accessibility` / build skills
