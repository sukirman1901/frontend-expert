...
Ship **indexable, shareable** pages. Don’t duplicate full LCP guides — pair with `performance` for CWV.
...
| **H1** | One primary heading; sequential ladder (`design-direction`) |
...
5. **CWV** — defer to `performance` / `devtools` for measured LCP/CLS
...
`references/seo.md`. Perf: `references/performance.md`. Type: `design-direction` Typography ladder.
