...
| `/design` or `/audit` | `design-reviewer` | anti-slop-design → polish → tokens → responsive → accessibility → performance → **motion** / **motion-inspection** (if animated) → **marketing** (if landing) (+ **fidelity** if mock; **devtools** if measuring) |
