...
|---------|-------|----------------|
| `/ui` (or chat) | `ui-developer` | judgment* → **foundations** → tokens → (+ **marketing** if marketing) → (+ shell/data/forms) → components → **responsive** → **motion** (light shell / `motion-families` hand-roll) → anti-slop-design → polish → accessibility |
