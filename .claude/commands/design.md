...
11. `frontend-judgment` when recommending redesign directions
12. `frontend-testing` — note coverage gaps as Info only
...
2. Scan for AI aesthetic anti-patterns (incl. decorative motion spam)
...
7. Check motion family budget + reduced-motion when animation present
8. Review accessibility against WCAG 2.1 AA
9. Note performance risks relevant to UI (LCP/INP/CLS); use `fe-devtools` when measuring
10. Output the design-reviewer scorecard (incl. Typography + Responsive + Motion) — mark unevaluated fields explicitly
11. End with prioritized next moves (max 5)
