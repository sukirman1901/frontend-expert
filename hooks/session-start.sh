...
jq -cn \
  --arg message "frontend-expert loaded (chat-first + FE lifecycle + quality loop · 30 skills).
...
- Build UI → design-direction* → design-foundations → tokens → (+ typography/color/surfaces/content owners as relevant) → (+ marketing if landing) → (+ shell/data/forms) → components → responsive → motion → anti-slop-design → polish → accessibility
