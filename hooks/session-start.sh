...
jq -cn \
  --arg message "frontend-expert loaded (chat-first + FE lifecycle + quality loop · 33 skills).
...
- Build UI → design-direction* → (+ prototype if comparing/stressing) → design-foundations → tokens → (+ typography/color/surfaces/content owners as relevant) → (+ marketing if landing) → (+ shell/data/forms) → components → anti-slop-code → responsive → motion → anti-slop-design → polish → accessibility
- Test → testing (+ devtools when measuring) (test-engineer)
- Audit → anti-slop-design → polish → tokens → design-foundations (if structural layout) → responsive → motion + motion-inspection (if animated) → marketing (if landing) → a11y → performance (+ fidelity / devtools as needed) [/design|/audit]
